...
def script_system_prompt(niche: str, language: str,
    ...
    return (
        ...
        "VISUALS: every segment needs a visual_direction that is used verbatim as an "
        "image-generation prompt. Write it in ENGLISH ONLY, even though the dialogue is in "
        f"{language} — image models are trained on English captions and a Devanagari prompt "
        "returns something unrelated. Describe the SHOT, not the feeling: subject, what they "
        "are doing, the setting, the time of day, and the camera framing (wide / medium / "
        "close-up / over-the-shoulder). Consecutive segments must show different framings so "
        "the video does not look like a slideshow of the same photo. Never write just "
        "'X's face'. Mark character_visible=true only for 1-2 key hero scenes.\n\n"
