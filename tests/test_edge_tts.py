...
def test_writes_audio_file(tmp_path, monkeypatch):
    t = EdgeTTS()
    monkeypatch.setattr(t, "_speak", lambda text, voice, dest: open(dest, "wb").write(b"\xff\xfb" + b"\x00" * 4096))
...
def test_male_and_female_map_to_different_hindi_voices(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(t, "_speak",
                        lambda text, voice, dest: (used.append(voice),
                                                   open(dest, "wb").write(b"\x00" * 2048))[1])
...
def test_empty_output_raises(tmp_path, monkeypatch):
    import pytest
    t = EdgeTTS()
    monkeypatch.setattr(t, "_speak", lambda text, voice, dest: open(dest, "wb").write(b""))
